...
helix_session_create(helix_protocol_t proto, helix_event_callback_t callback, void* data)
{
  ...
  session->register_callback([session, callback](std::shared_ptr<helix::event> event) {
    callback(wrap(session), wrap(event.get()));
                             });
  return wrap(session);
}
...
bool helix::session::check_is_working_time(uint64_t timestamp) {
  auto fut = post(_pool, use_future(
    [=] {
      return this->is_rth_timestamp(timestamp);
    }));
  return fut.get();
}

size_t helix::session::received_packet(const net::packet_view& packet)
{
  try
  {
    if (support_async_packet_handling)
    {
      auto payload_len = this->get_packet_length(packet);
      if (payload_len == 0) {
        // End of session.
        return 0;
      }
      ...
      std::vector<char> receive_buffer{ packet.buf(), packet.buf() + payload_len };

      // þimdilik asenkron ama senkron olarak sonucu döndürüyor. 
      // sonrasýnda buradaki future larý bir yerde toplayabilir, 
      // asio servis ile proactor bir yapý kurabiliriz
      post(_pool,
           [=, buff = std::move(receive_buffer)]{
              this->process_packet(net::packet_view{buff.data(), buff.size()});
           });
      return payload_len;
    }
    else
    {
      auto len = post(_pool, use_future(
           [=]{
              return this->process_packet(packet);
           }));
      return len.get();
    }
  }
